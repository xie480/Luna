...
#### 规则 3：其余请求做单源 / 多源分流
- `source_count == 1` -> `native`
- `source_count > 1` -> `modular`

特征：
- `native`：单主源即可回答，不需要跨表整合
- `modular`：同时依赖两类及以上 source（如“结合我之前的情况”“按我的偏好”）

#### 规则 4：受 allowed_routes 约束
- 若命中 `agentic` 但 `allowed_routes` 不包含 `agentic`，则自动降级到规则 3 分流。
