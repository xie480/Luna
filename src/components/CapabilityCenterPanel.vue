...
async function handleSelect(item) {
  ...
  if (activeView.value === "workflowTemplate") writeEditors.workflowTemplate = stringifyPretty({ workflowName: item.name || "", description: item.description || "", inputSchema: safeJsonParse(item.inputSchema, {}), outputSchema: safeJsonParse(item.outputSchema, {}), requiredCapabilities: ensureArray(item.requiredCapabilities), toolSlots: ensureArray(item.toolSlots), thoughtChain: ensureArray(item.thoughtChain), blueprintJson: safeJsonParse(item.blueprintJson, {}), version: item.version || "", enabled: true });
