...
/**
 * 蓝图校验服务实现，负责校验主规划阶段生成的计划蓝图结构是否合法，避免无效蓝图进入执行链路。
 */
@Service
public class BlueprintValidationServiceImpl implements BlueprintValidationService {
    ...
    private static final Set<String> VALID_NODE_TYPES = Set.of(
            "ANALYZE", "TOOL", "VALIDATE", "REPORT", "CODE",
    ...
    public String validate(Map<String, Object> blueprint) {
        /**
         * 先校验蓝图顶层必填信息，避免后续阶段在缺少主键字段时继续深入解析。
         */
        if (blueprint == null) {
            return "blueprint 不能为空";
        }
        if (isBlank(asStr(blueprint.get("planId")))) {
            return "planId 不能为空";
        }
        if (isBlank(asStr(blueprint.get("sessionId")))) {
            return "sessionId 不能为空";
        }
        if (isBlank(asStr(blueprint.get("userGoal")))) {
            return "userGoal 不能为空";
        }

        /**
         * 校验阶段列表的完整性，并顺带收集阶段 ID 与顺序，为节点校验提供合法范围。
         */
        ...
        for (Object p : phases) {
            ...
            if (isBlank(name)) {
                return "phase.name 不能为空";
            }
            if (isBlank(objective)) {
                return "phase.objective 不能为空";
            }
            if (order == null || order < 1) {
                return "phase.phaseOrder 必须为正整数";
            }

            if (!phaseIds.add(phaseId)) {
                return "phaseId 重复: " + phaseId;
            }
            if (!phaseOrders.add(order)) {
                return "phaseOrder 重复: " + order;
            }
        }

        /**
         * 校验节点列表及其类型、风险等级和阶段归属，确保执行图的主体节点可被正确调度。
         */
        ...
        for (Object n : nodes) {
            ...
            if (!nodeIds.add(nodeId)) {
                return "nodeId 重复: " + nodeId;
            }
            if (isBlank(phaseId)) {
                return "node.phaseId 不能为空";
            }
            if (!phaseIds.contains(phaseId)) {
                return "node.phaseId 不存在: " + phaseId;
            }
