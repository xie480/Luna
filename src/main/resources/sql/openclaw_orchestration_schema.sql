...
COMMENT ON COLUMN plan_instance.status IS '计划状态：PENDING/RUNNING/WAITING_USER_APPROVAL/SUCCESS/FAILED/CANCELLED';
COMMENT ON COLUMN plan_instance.current_loop_index IS '当前loop轮次';
COMMENT ON COLUMN plan_instance.final_status IS '最终状态';
...
COMMENT ON COLUMN plan_phase.status IS '阶段状态：PENDING/RUNNING/SUCCESS/FAILED';
...
COMMENT ON COLUMN plan_node.node_type IS '节点类型：ANALYZE/TOOL/SKILL/VALIDATE/SUMMARIZE/REPORT/CODE';
COMMENT ON COLUMN plan_node.dependencies IS '依赖节点ID列表(JSON)';
COMMENT ON COLUMN plan_node.parallel_group IS '并行组标识';
COMMENT ON COLUMN plan_node.status IS '节点状态：PENDING/RUNNING/SUCCESS/FAILED/BLOCKED/APPROVAL_PENDING/SKIPPED';
COMMENT ON COLUMN plan_node.retry_policy IS '重试策略(JSON)';
COMMENT ON COLUMN plan_node.model_hint IS '模型提示：small/mid/big/flash';
COMMENT ON COLUMN plan_node.resource_hint IS '候选资源提示(JSON)';
COMMENT ON COLUMN plan_node.output_for_next IS '传递给下游节点的关键数据';
COMMENT ON COLUMN plan_node.risk_level IS '风险等级：LOW/MEDIUM/HIGH';
...
COMMENT ON COLUMN plan_event_log.event_type IS '事件类型：PLAN_CREATED/PLAN_NODE_SUCCESS/PLAN_TEST_RESULT/...';
...
COMMENT ON COLUMN plan_report.open_result IS '浏览器唤起结果：SUCCESS/FAILED';
...
-- =========================================================
-- 可选：状态约束（保守处理，避免过度收紧影响扩展）
-- =========================================================
-- 你可按需要启用更严格 CHECK 约束
-- ALTER TABLE plan_instance ADD CONSTRAINT ck_plan_instance_status
-- CHECK (status IN ('PENDING','RUNNING','WAITING_USER_APPROVAL','SUCCESS','FAILED','CANCELLED'));

-- ALTER TABLE plan_phase ADD CONSTRAINT ck_plan_phase_status
-- CHECK (status IN ('PENDING','RUNNING','SUCCESS','FAILED'));

-- ALTER TABLE plan_node ADD CONSTRAINT ck_plan_node_status
-- CHECK (status IN ('PENDING','RUNNING','SUCCESS','FAILED','BLOCKED','APPROVAL_PENDING','SKIPPED'));
