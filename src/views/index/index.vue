...
async function performLogin() {
  ...
  try {
    ...
    // 【修复】在移除 UI 前强制重置穿透状态，防止鼠标被透明层阻挡
    overUI = false;
    updatePetState();
...
function onPointerMove(e) {
  if (!dragging || !isSetupMode.value) return;
...
/* ================= 滚轮缩放 ================= */
function onWheel(ev) {
  if (!model || !app || !isSetupMode.value) return;
